            ...
            <div className="flex flex-col lg:flex-row md:justify-start items-center">
                <div className="flex flex-col items-center md:flex-row">
                    <div className="image md:shrink-0 bg-blue-500 h-60 w-56 rounded-lg">
                        <img className='h-full w-full object-cover rounded-lg' src={details.imagePath?`http://localhost:5000/images/doctors/${details.imagePath}`:null} alt="" />
                    </div>
                ...
                <div className='h-60 w-full lg:w-2/5'>
                        ...
                        <table className="min-w-full bg-blue-200 text-center text-sm font-light rounded-t-lg">
                            <thead className="border-b font-medium dark:border-neutral-500">
                                <tr>
                                    ...
                                    <th scope="col" className="px-8 py-4">Last</th>
                                        ...
                                        (available.length>0)?(
                                            available.map((date,index)=>(
                                                <tr key={index} className="border-b dark:border-neutral-500">
                                                    <td className="whitespace-nowrap px-3 py-4 font-medium">{index+1}</td>
                                                    <td className="whitespace-nowrap  py-4">{date.date}</td>
                                                    <td className="whitespace-nowrap px-4 py-4">{convertTo12HourFormat(date.fromTime)}</td>
                                                    <td className="whitespace-nowrap px-6 py-4">{convertTo12HourFormat(date.toTime)}</td>
                                                </tr>
                                            ))
                                        ):(
                                            <tr className="border-b dark:border-neutral-500">
                                                <td colSpan={4} className="whitespace-nowrap px-3 py-4 font-medium">No Special Timings</td>
                                            </tr>
                                        )
                                    }
                                </tbody>
                    ...
                    <h6 className='font-bold text-xs text-blue-500'>Normal timings will be from 10 am to 8 pm. (Breaks excluded)</h6>
                </div>
            </div>
